  ...
  <section bg-neutral-0>
    <div grid="~ gap-x-6 flow-row gap-y-20" :style="{ gridTemplateColumns, gridTemplateRows: `repeat(${slice.primary.item.length + 1}, auto)` }" w-full>
      ...
      <template v-for="({ title, description, color, percentage }, i) in slice.primary.item" :key="i">
          ...
          <AccordionItem col-span-full :value="title!">
            <AccordionHeader>
              <AccordionTrigger flex="~ items-center gap-x-8" w-full bg-transparent>
                ...
                <div v-if="i > 0" relative bottom--2.7 right--6 h-1.5 flex-1 transition-colors bg="neutral-600 data-open:neutral-700" @click.stop />
        ...
        <div v-for="j in Math.max(i - 1, 0)" :key="j" w="[calc(100%+6px)]" :data-state="activeIndexes.includes(Math.max(i - 1, 0) + 1) ? 'active' : undefined" relative bottom--13 h-1.5 transition-colors bg="neutral-600 data-active:neutral-700" />
        ...
        <div v-for="j in slice.primary.item.length - i - 1" :key="j" h="[calc(100%+40px)]" :data-state="activeIndexes.includes(j + i) ? 'active' : undefined" w-1.5 self-center justify-self-center bg="neutral-600 data-active:neutral-700" />
      </template>
    </div>
  </section>
</template>
